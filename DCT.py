...
class DCT():
    ...
    def is_grey_scale(self):
        img1 = Image.open(self.imPath).convert('RGB')
        w,h = img1.size
        for i in range(w):
            for j in range(h):
    ...
    def DCTEn(self, secret, outIm):
        ...
        if self.is_grey_scale() and len(img.shape) == 3:
            colored_grayscale = 1
            print("Colored Grayscale")
...
#        print(cv2.split(img))
        ...
        c_img = ""
        if colored_grayscale == 1:
            bImg, gImg, rImg, aImg = cv2.split(img)
#            aImg, gImg, rImg, bImg = cv2.split(img)
        elif grayscale == 1:
            bImg = img
        else:
...
#            aImg, gImg, rImg, bImg = cv2.split(img_BGRA)
#            c_img = bImg
        ...
        if colored_grayscale == 1:
            sImg = cv2.merge((sImg, gImg, rImg, aImg))
#            sImg = cv2.merge((aImg, gImg, rImg, sImg))
        elif grayscale == 1:
            print("Grayscaled")
        else:
#            sImg = cv2.merge((aImg, gImg, rImg,sImg))
#            cv2.imwrite(self.imPath, cv2.merge((aImg, gImg, rImg,c_img)))
            sImg = cv2.merge((sImg, gImg, rImg))

        #if grayscale_flag:
        #    sImg = cv2.cvtColor(gray,cv2.COLOR_BGR2GRAY)
        img = self.loadImage()
        print("Checking")
        print(img.shape, sImg.shape)
        cv2.imwrite(outIm, sImg)
        cv2.imwrite(self.imPath, img)
        return sImg
...
#https://www.gizbot.com/news/how-indian-cyber-security-startups-are-growing-list-of-startups-064703.html
